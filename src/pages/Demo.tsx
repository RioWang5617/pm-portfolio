  ...
  if (!d) {
    return (
      <div className="mx-auto max-w-prose px-6 md:px-10 py-32 text-center">
        ...
        <Link to="/vibecoding" className="mt-8 inline-block underline">
          返回 →
        </Link>
      </div>
    )
    ...
    <article>
      ...
      <div className="mx-auto max-w-wide px-6 md:px-10 mb-16 reveal">
        {d.embed ? (
          ...
          <div
            className={`aspect-[16/9] w-full rounded-sm bg-gradient-to-br ${d.cover} flex items-center justify-center`}
          >
            <p className="font-display italic text-cream/85 text-[1.4rem] md:text-[2rem] tracking-tightest">
              {d.title}
            </p>
          </div>
        )}
        ...
        {d.sourceUrl && (
          <p className="mt-6">
            <a
              ...
              target="_blank"
              rel="noreferrer"
              className="text-[0.95rem] underline decoration-line underline-offset-[6px] hover:decoration-ink"
            >
              查看源码 →
            </a>
          </p>
